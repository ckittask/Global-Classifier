...
try:
    ...
    if script_dir.exists():
        ...
        if services_dir.exists():
            logger.error(
                f"Contents of services directory: {list(services_dir.iterdir())}"
            )
    traceback.print_exc()
    sys.exit(1)
...
def chunk_dataset(dataset_path: str, chunk_size: int = 5):
    ...
    try:
        ...
        for i in range(0, total_items, chunk_size):
            ...
            logger.info(
                f"[CHUNKING] Created chunk {chunk_number}: {chunk_filename} ({len(chunk_data)} items)"
            )

        logger.info(
            f"[CHUNKING] ✅ Created {len(chunk_files)} chunk files in: {chunks_dir}"
        )
        return chunk_files
...
def upload_chunks_to_s3(chunk_files: list, dataset_id: str):
    ...
    try:
        logger.info(
            f"[S3_UPLOAD] Starting S3 upload for {len(chunk_files)} chunks using S3Ferry service"
        )
        ...
        for chunk_file in chunk_files:
            ...
            try:
                ...
                logger.info(
                    f"[S3_UPLOAD] S3Ferry response status: {response.status_code}"
                )
                ...
                if response.status_code in [200, 201]:
                    ...
                    logger.info(
                        f"[S3_UPLOAD] ✅ Uploaded: {chunk_filename} ->  (HTTP {response.status_code})"
                    )
                else:
                    upload_results.append(
                        {
                    ...
                    logger.error(
                        f"[S3_UPLOAD] ❌ Failed to upload {chunk_filename}: HTTP {response.status_code}"
                    )
                ...
                logger.error(
                    f"[S3_UPLOAD] ❌ Request failed for {chunk_filename}: {str(e)}"
                )
                traceback.print_exc()
                ...
                logger.error(
                    f"[S3_UPLOAD] ❌ Unexpected error for {chunk_filename}: {str(e)}"
                )
                traceback.print_exc()
        ...
        logger.info(
            f"[S3_UPLOAD] ✅ Upload complete: {len(successful_uploads)} successful, {len(failed_uploads)} failed"
        )
        ...
        if successful_uploads:
            ...
            for result in successful_uploads:
                ...
                logger.info(
                    f"[S3_UPLOAD]   - {result['chunk_file']}: {result['s3_url']} (HTTP {status_code})"
                )
        ...
        if failed_uploads:
            ...
            for result in failed_uploads:
                logger.warning(
                    f"[S3_UPLOAD]   - {result['chunk_file']}: {result['error']}"
                )
...
def update_chunk_metadata(chunk_file_path: str, dataset_id: str, chunk_number: int):
    ...
    try:
        logger.info(
            f"[CHUNK_METADATA] Updating metadata for chunk {chunk_number} of dataset {dataset_id}"
        )
        ...
        logger.info(
            f"[CHUNK_METADATA] Extracted {len(agency_ids)} agency IDs: {agency_ids}"
        )
        ...
        logger.info(
            f"[CHUNK_METADATA] Payload: {json.dumps(metadata_payload, indent=2)}"
        )

        # Send POST request to the chunk metadata endpoint
        CHUNK_METADATA_URL = (
            "http://resql:8082/global-classifier/update-data-chunk-metadata"
        )
        ...
        if response.status_code == 200:
            logger.info(
                f"[CHUNK_METADATA] ✅ Successfully updated metadata for chunk {chunk_number}"
            )
        else:
            logger.warning(
                f"[CHUNK_METADATA] ⚠️ Metadata update failed for chunk {chunk_number}: HTTP {response.status_code}"
            )
...
def process_callback_background(file_path: str, encoded_results: str):
    ...
    try:
        ...
        if os.path.exists(full_dataset_path):
            ...
            logger.info(
                "[CALLBACK] 📤 Starting upload of original aggregated dataset..."
            )
            try:
                ...
                logger.info(
                    f"[CALLBACK] Original dataset upload status: {original_dataset_response.status_code}"
                )
                logger.info(
                    f"[CALLBACK] Original dataset upload response: {original_dataset_response.text}"
                )

                if original_dataset_response.status_code in [200, 201]:
                    original_s3_url = (
                        f"s3://global-classifier/{dataset_id}/aggregated_dataset.json"
                    )
                    logger.info(
                        f"[CALLBACK] ✅ Original dataset uploaded: {original_s3_url}"
                    )

                else:
                    logger.error(
                        f"[CALLBACK] ❌ Failed to upload original dataset: HTTP {original_dataset_response.status_code}"
                    )

            except Exception as e:
                logger.error(
                    f"[CALLBACK] ❌ Error uploading original dataset: {str(e)}"
                )
                traceback.print_exc()
            ...
            if chunk_files:
                ...
                # Log upload summary
                successful_uploads = [
                    r for r in upload_results if r.get("success", False)
                ]
                logger.info("[CALLBACK] 📊 S3 Upload Summary:")
                logger.info(f"[CALLBACK]   - Total chunks: {len(chunk_files)}")
                logger.info(
                    f"[CALLBACK]   - Successful uploads: {len(successful_uploads)}"
                )
                logger.info(
                    f"[CALLBACK]   - Failed uploads: {len(upload_results) - len(successful_uploads)}"
                )

                # Log S3 URLs
                for result in successful_uploads:
                    logger.info(
                        f"[CALLBACK]   - {result['chunk_file']}: {result['s3_url']}"
                    )
                ...
                for i, chunk_file in enumerate(chunk_files):
                    ...
                    if upload_success:
                        try:
                            ...
                            logger.error(
                                f"[CALLBACK] ❌ Failed to update metadata for chunk {chunk_number}: {str(e)}"
                            )
                            metadata_results.append(
                                {
                                    "chunk_number": chunk_number,
                        ...
                        logger.warning(
                            f"[CALLBACK] ⚠️ Skipping metadata update for chunk {chunk_number} (upload failed)"
                        )
                        metadata_results.append(
                            {
                                "chunk_number": chunk_number,
...
def main():
    ...
    parser.add_argument(
        "--file-path", required=True, help="File path of the generated dataset"
    )
    parser.add_argument(
